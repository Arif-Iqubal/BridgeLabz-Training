...
public class MedWarehouse {
	...
	static int listSize = 0; // Static variable to store size of Medicine
	static Medicine[] medicineList = new Medicine[listSize]; // Medicine array to store Medicine object

	// Static method to add Medicine to array list
	public static void addMedicine(String medicineName, LocalDate mfg, LocalDate expiry) {
		...
		if (listSize <= medicineList.length) {
			...
			for (int i = 0; i < listSize - 1; i++) {
				temp[i] = medicineList[i];
			}
			temp[listSize - 1] = Medicine;
			medicineList = temp;
			System.out.println(listSize + ") Medicine added successfully ");
		}
