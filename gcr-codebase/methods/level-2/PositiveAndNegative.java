...
class PositiveAndNegative {
	...
	//Method to check number is positive or not
	...
	//Method to comapre first and last number
   ...
   public static void main(String[] args) {
      ...
      for(int i = 0 ; i < arrayLength ; i++ ){
		  boolean isPositiveNumber = PositiveAndNegative.isPositive(arr[i]);
		  ...
		  if(!isPositiveNumber){
			System.out.println("The number " + arr[i] + " is negative.");
			   ...
			   boolean isEvenNumber = PositiveAndNegative.isEven(arr[i]);
			   if( isEvenNumber ){
	  ...
	  int firstLastCompare = PositiveAndNegative.compare(arr[0], arr[arrayLength-1]);
	  if(firstLastCompare == 0){
		System.out.println("The first value " + arr[0] + " and the last value " + arr[arrayLength-1] + " are equal.");

	  }
	  else if(firstLastCompare == -1){
